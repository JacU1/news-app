...
export function startGlobe(globeContainer) {
let scene = new THREE.Scene();
let camera = new THREE.PerspectiveCamera(45, globeContainer.clientWidth / globeContainer.clientHeight, 1, 2000);
camera.position.set(0.5, 0.5, 1).setLength(14);
let renderer = new THREE.WebGLRenderer({
  antialias: true
});
renderer.setSize(globeContainer.clientWidth, globeContainer.clientHeight);
renderer.setClearColor(0x0e1111, 0);
globeContainer.appendChild(renderer.domElement);

let labelRenderer = new CSS2DRenderer();
labelRenderer.setSize(globeContainer.clientWidth, globeContainer.clientHeight);
labelRenderer.domElement.style.position = 'absolute';
labelRenderer.domElement.style.inset = '0';
labelRenderer.domElement.style.pointerEvents = 'none';
globeContainer.appendChild(labelRenderer.domElement);
...
const textureUrl = new URL('assets/globe-texture.jpg', document.baseURI).href;
const globeTexture = new THREE.TextureLoader().load(textureUrl);
...
closeBtn.addEventListener("pointerdown", closeLabel);
...
return () => {
  ...
  closeBtn.removeEventListener("pointerdown", closeLabel);
  controls.dispose();
  globe.geometry.dispose();
  globe.material.dispose();
  icshdrn.geometry.dispose();
  icshdrn.material.dispose();
  gMarker.dispose();
  mMarker.dispose();
